...
def ph_req_battery_data(ham, module_id, wait_on_response_timeout=DEFAULT_WAIT_ON_RESPONSE_TIMEOUT):
    ...
    data = ham.wait_on_response(cmd, raise_first_exception=True, timeout=wait_on_response_timeout, return_data = return_fields)
    return data

def ph_measure(ham, module_id, temperature, probePattern, wait_on_response_timeout=DEFAULT_WAIT_ON_RESPONSE_TIMEOUT):
    return_fields = ['step-return2', 'step-return3', 'step-return4', 'step-return5']
    cmd = ham.send_command(PH_MEASURE, ModuleID = module_id, Temperature = temperature, probePattern = probePattern)
    data = ham.wait_on_response(cmd, raise_first_exception=True, timeout=wait_on_response_timeout, return_data = return_fields)
    return data

def ph_measure_dynamic(ham, module_id, temperature, precision, timeout, probePattern, wait_on_response_timeout=DEFAULT_WAIT_ON_RESPONSE_TIMEOUT):
    return_fields = ['step-return2', 'step-return3', 'step-return4', 'step-return5']
...
def ph_request_calibration(ham, module_id, probe_number, wait_on_response_timeout=DEFAULT_WAIT_ON_RESPONSE_TIMEOUT):
    return_fields = ['step-return2', 'step-return3', 'step-return4', 'step-return5',
...
def ph_request_probe_data(ham, module_id, wait_on_response_timeout=DEFAULT_WAIT_ON_RESPONSE_TIMEOUT):
    return_fields = ['step-return2', 'step-return3', 'step-return4', 'step-return5',
                     'step-return6'
                     ]
...
def ph_request_technical_data(ham, module_id, hardware_number, wait_on_response_timeout=DEFAULT_WAIT_ON_RESPONSE_TIMEOUT):
    return_fields = ['step-return2', 'step-return3', 'step-return4', 'step-return5']
    cmd = ham.send_command(PH_REQ_TECH_DATA, ModuleID = module_id, HardwareNumber = hardware_number)
    data = ham.wait_on_response(cmd, raise_first_exception=True, timeout=wait_on_response_timeout, return_data = return_fields)
    return data

def ph_calibrate(ham, module_id, cal_level, cal_value, cal_temperature, probe_pattern, wait_on_response_timeout=DEFAULT_WAIT_ON_RESPONSE_TIMEOUT):
    ...
    data = ham.wait_on_response(cmd, raise_first_exception=True, timeout=wait_on_response_timeout)
    return data

def ph_calibrate_dynamically(ham, module_id, variance, timeout, cal_level, cal_value, cal_temperature, probe_pattern, wait_on_response_timeout=DEFAULT_WAIT_ON_RESPONSE_TIMEOUT):
...
def ph_wakeup(ham, module_id, wait_on_response_timeout=DEFAULT_WAIT_ON_RESPONSE_TIMEOUT):
    cmd = ham.send_command(PH_WAKEUP, ModuleID = module_id)
    ham.wait_on_response(cmd, raise_first_exception=True, timeout=wait_on_response_timeout)

def ph_sleep(ham, module_id, wait_on_response_timeout=DEFAULT_WAIT_ON_RESPONSE_TIMEOUT):
    cmd = ham.send_command(PH_SLEEP, ModuleID = module_id)
    ham.wait_on_response(cmd, raise_first_exception=True, timeout=wait_on_response_timeout)

def ph_washer_initialize(ham, comport, simulate, wait_on_response_timeout=DEFAULT_WAIT_ON_RESPONSE_TIMEOUT):
    cmd = ham.send_command(PH_WASHER_INIT, Comport = comport, SimulationMode = simulate)
    module_id = ham.wait_on_response(cmd, raise_first_exception=True, timeout=wait_on_response_timeout, return_data=['step-return2'])
    return module_id

def ph_washer_wash(ham, module_id, cycle_num, wait_on_response_timeout=DEFAULT_WAIT_ON_RESPONSE_TIMEOUT):
    cmd = ham.send_command(PH_WASHER_WASH, ModuleID = module_id, CycleNumber = cycle_num)
    ham.wait_on_response(cmd, raise_first_exception=True, timeout=wait_on_response_timeout)

def ph_washer_terminate(ham, module_id, wait_on_response_timeout=DEFAULT_WAIT_ON_RESPONSE_TIMEOUT):
    cmd = ham.send_command(PH_WASHER_TERM, ModuleID = module_id)
    ham.wait_on_response(cmd, raise_first_exception=True, timeout=wait_on_response_timeout)

def ph_dryer_initialize(ham, comport, simulate, wait_on_response_timeout=DEFAULT_WAIT_ON_RESPONSE_TIMEOUT):
    cmd = ham.send_command(PH_DRYER_INIT, Comport = comport, SimulationMode = simulate)
    module_id = ham.wait_on_response(cmd, raise_first_exception=True, timeout=wait_on_response_timeout, return_data=['step-return2'])
    return module_id

def ph_dryer_start(ham, module_id, wait_on_response_timeout=DEFAULT_WAIT_ON_RESPONSE_TIMEOUT):
    cmd = ham.send_command(PH_DRYER_START, ModuleID = module_id)
    ham.wait_on_response(cmd, raise_first_exception=True, timeout=wait_on_response_timeout)

def ph_dryer_stop(ham, module_id, wait_on_response_timeout=DEFAULT_WAIT_ON_RESPONSE_TIMEOUT):
    cmd = ham.send_command(PH_DRYER_STOP, ModuleID = module_id)
    ham.wait_on_response(cmd, raise_first_exception=True, timeout=wait_on_response_timeout)

def ph_dryer_terminate(ham, module_id, wait_on_response_timeout=DEFAULT_WAIT_ON_RESPONSE_TIMEOUT):
